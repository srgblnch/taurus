...
public class AddChannelDialog extends JDialog 
{
	...
	{
		super();
		setTitle("Create Experiment Channel");
	}
			
	public AddChannelDialog(DevicePool devicePool)
	{
		this();
	...
	public AddChannelDialog(DevicePool devicePool, ExperimentChannel channel)
	{
		this();
		this.devicePool = devicePool;
	...
	void updateControllers()
	{
		...
		for(Controller ctrl : devicePool.getControllers() )
			if(ctrl.getType() == ControllerType.valueOf(type))
	...
	protected void init()
	{
		...
		typeCombo = new JComboBox(new Object [] { 
				ControllerType.CounterTimer.toString(), 
				ControllerType.ZeroDExpChannel.toString() });
		
		if(channel != null)
		{
			typeCombo.setSelectedItem(channel.getController().getType().toString());
		...
		createButton.setToolTipText("Create a new experiment channel");
	...
	protected void createPressed(ActionEvent e)
	{
		...
		try
		{
			...
			args.insert(params);
			devicePool.getDevice().executeCommand(DevicePoolUtils.POOL_CMD_CREATE_EXP_CHANNEL,args);
			
			JOptionPane.showMessageDialog(this,
					"Channel " + name + " sucessfully created","Sucess!", JOptionPane.INFORMATION_MESSAGE);

		}
		catch (DevFailed devFailed)
		{
			log.fine("User error trying to create Channel with: " + ctrl.getName() + ", " + index + ", " + name);
			...
			JOptionPane.showMessageDialog(this, 
					buff.toString(),
					"Error trying to create a new channel", 
	...
	class ChannelNumberModel extends SpinnerNumberModel implements ActionListener 
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			{
				ExperimentChannel motor = (ExperimentChannel)elem;
				invalidChannelNumbers[i++] = motor.getIdInController();
			}
