...
public class AddControllerDialog extends JDialog implements IStringSpectrumListener
{
	...
	private JTextField pseudoCounterName;
	...
	protected void initComponents()
	{
        ...
        pseudoCounterRolesPanel.setBorder(javax.swing.BorderFactory.createTitledBorder("Pseudo Counter"));
        ...
        constraints.gridx = 0;
        constraints.gridy = 0;
		JLabel pcRole = new JLabel("Name:");
		pcRole.setToolTipText("The pseudo counter name");
		pseudoCounterRolesPanel.add(pcRole, constraints);
		
		constraints.anchor = GridBagConstraints.EAST;
		constraints.weightx = 1.0;
		constraints.gridx = 1;
		pseudoCounterName = new JTextField();
		pseudoCounterName.setToolTipText("The name of the pseudo counter");
		pseudoCounterRolesPanel.add(pseudoCounterName, constraints);
        
        pseudoCounterDataPanel.add(pseudoCounterRolesPanel);
        
        counterRolesPanel = new JPanel();
        counterRolesPanel.setLayout(new GridBagLayout());
        pseudoCounterRolesPane = new JScrollPane(counterRolesPanel);
        pseudoCounterRolesPane.setBorder(javax.swing.BorderFactory.createTitledBorder("Counter Roles"));
	...
	protected void updatePseudoCounterData(PseudoCounterClass pcClass)
	{
		...
		if(pcClass != null)
		{
			...
			for(String pcRoles : pcClass.getCounterRoles())
			{
				...
				counterNames[i] = new JComboBox();
				for(ExperimentChannel ch : devicePool.getExperimentChannels())
				{
					if(ch instanceof CounterTimer || ch instanceof ZeroDExpChannel)
						counterNames[i].addItem(ch);
				}
				
				counterNames[i].setToolTipText("The name of the counter which will have the corresponding role in the pseudo counter system");
				counterRolesPanel.add(counterNames[i], gbc);
				i++;
			}			
		}
	}
	...
	protected void createPressed(ActionEvent e)
	{
		...
		{
			String pseudoCounterNameStr = pseudoCounterName.getText();
			if(pseudoCounterNameStr == null || pseudoCounterNameStr.length() == 0)
			{
		...
		if(ctrlType == ControllerType.PseudoMotor)
			...
			paramCount += tmpCounterNames.size() + 1; // +1 for the pseudocounter name
		...
		try
		{
			...
			else if(ctrlType == ControllerType.PseudoCounter)
			{
				for(String cName : tmpCounterNames)
					params[i++] = cName;
				params[i++] = pseudoCounterName.getText();
			}	
	...
	public static void main(String[] args) 
	{
		...
		// 5
		c = new PseudoCounterClass("Type: PseudoCounter - Class: Current - File: /home/tiago/Current.py");
		c.setDescription("A current pseudo counter (I/I0)\n");
