...
class SerialComCtrl(CommunicationController):
    ...
    ctrl_extra_attributes = {
       ...
       'Terminator' : {'Type':'PyTango.DevLong',   'R/W Type':'PyTango.READ_WRITE'},
...
class SerialRemoteTangoComCtrl(SerialComCtrl):
    ...
    def AddDevice(self,ind):
        print "[SerialRemoteTangoComCtrl]",self.inst_name,": In Adddevice for [",ind,"]"
    ...
    def StateOne(self,ind):
        print "[SerialRemoteTangoComCtrl]",self.inst_name,": In StateOne method for [",ind,"]"
...
class SerialRemotePyTangoComCtrl(SerialRemoteTangoComCtrl):
    """A Sardana serial line communication controller for a remote serial line through a 
       PyTango serial line device server"""
    ...
    def OpenOne(self,ind):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In OpenOne for [",ind,"]"
        sd = self.get_serial_device_data(ind)
        if sd.proxy.state() == PyTango.DevState.OFF:
            sd.proxy.open()
    
    def CloseOne(self,ind):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In CloseOne for [",ind,"]"
        sd = self.get_serial_device_data(ind)
        if sd.proxy.state() != PyTango.DevState.OFF:
            sd.proxy.close()
    
    def ReadOne(self,ind,max_read_len):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In ReadOne for [",ind,"]. max_read_len = ",max_read_len
        sd = self.get_serial_device_data(ind)
    ...
    def ReadLineOne(self,ind):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In ReadLineOne for [",ind,"]"
        sd = self.get_serial_device_data(ind)
    ...
    def WriteOne(self,ind,buf,write_len):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In WriteOne for [",ind,"]. buf = [", buf, "] write_len = ",write_len 
        sd = self.get_serial_device_data(ind)
    ...
    def WriteReadOne(self,ind,buf,write_len,max_read_len):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In WriteReadOne for [",ind,"]. buf = [", buf, "] write_len = ",write_len, ", max_read_len = ", max_read_len
        sd = self.get_serial_device_data(ind)
        if sd.proxy.state() != PyTango.DevState.ON:
            return ""
    ...
    def GetExtraAttributePar(self,ind,name):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In GetExtraAttributePar for [",ind,"]. Name = ",name
        ...
        if name == "Device":
            ...
            return sd.proxy.read_attribute("Timeout").value

    def SetExtraAttributePar(self,ind,name,value):
        print "[SerialRemotePyTangoComCtrl]",self.inst_name,": In SetExtraAttributePar for [",ind,"]. Name = ",name, "Value = ",value
    ...
    def __del__(self):
        print "[SerialRemotePyTangoComCtrl] ",self.inst_name,": Aarrrrrg, I am dying"
