...
void EchoCommunicationController::AddDevice(long idx)
{
...
void EchoCommunicationController::DeleteDevice(long idx)
{
...
string &EchoCommunicationController::ReadOne(long idx, long max_read_len /* = -1*/)
{
...
string &EchoCommunicationController::ReadLineOne(long idx)
{
	if(state[idx-1] == ChDisabled)
	{
		Tango::Except::throw_exception(
...
long EchoCommunicationController::WriteOne(long idx, string &istr, long write_len /*= -1*/)
{	
	if(state[idx-1] == ChDisabled)
	{
		Tango::Except::throw_exception(
...
string &EchoCommunicationController::WriteReadOne(long idx, string &istr, long write_len /*= -1*/, long max_read_len /*= -1*/)
{	
	if(state[idx-1] == ChDisabled)
	{
		Tango::Except::throw_exception(
...
void EchoCommunicationController::StateOne(long idx,Controller::CtrlState *ch_info_ptr)
{
...
Controller::CtrlData EchoCommunicationController::GetExtraAttributePar(long idx,string &par_name)
{
...
void EchoCommunicationController::SetExtraAttributePar(long idx,string &par_name,Controller::CtrlData &new_value)
{
	TangoSys_OMemStream o;
	o << "Parameter " << par_name << " is unknown for controller EchoCommunicationController/" << get_name() << ends;
...
long EchoCommunicationController_MaxDevice = 100;
