...
const char *Diffrac2C_pseudo_motor_roles[] = { "H", "K", NULL };
const char *Diffrac2C_motor_roles[] = { "tth", "th", NULL };

long Diffrac2C_MaxDevice = 2; // should be the pseudo motor role #
...
Controller::PropInfo Diffrac2C_class_prop[] = {
		...
		{"reflections", "list of reflections. Each reflection is a list of 5 numbers (space separated): th tth H K L", "DevVarStringArray",""},
		{NULL} };
...
double Diffrac2C::CalcPhysical(long axis,vector<double> &pseudo_pos)
{
 ...
 */
double Diffrac2C::CalcPseudo(long axis,vector<double> &physical_pos)
{
...
string Diffrac2C::calcA(vector<string> &params)
{
	...
	try
	{
		...
		tth = g.tth()->get_current().get_value() * hkl::constant::math::radToDeg;
		th = g.omega()->get_current().get_value() * hkl::constant::math::radToDeg;
