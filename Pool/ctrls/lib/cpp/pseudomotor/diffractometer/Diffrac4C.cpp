...
double Diffrac4C::CalcPhysical(long axis,vector<double> &pseudo_pos)
{
 ...
 */
double Diffrac4C::CalcPseudo(long axis,vector<double> &physical_pos)
{
...
cout << "[Diffrac4C] Entering CalcAllPhysical(H=" << h << ", K=" << k 
	...
	try
	{
		...
		physical_pos[0] = tth = g.tth()->get_current().get_value() * hkl::constant::math::radToDeg;
		physical_pos[1] = th = g.omega()->get_current().get_value() * hkl::constant::math::radToDeg;
		physical_pos[2] = chi = g.chi()->get_current().get_value() * hkl::constant::math::radToDeg;	
		physical_pos[3] = phi = g.phi()->get_current().get_value() * hkl::constant::math::radToDeg;
