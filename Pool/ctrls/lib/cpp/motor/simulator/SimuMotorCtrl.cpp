...
void SimuMotorController::AddDevice(long idx)
{
...
void SimuMotorController::DeleteDevice(long idx)
{
...
void SimuMotorController::AbortOne(long idx)
{
...
void SimuMotorController::DefinePosition(long idx,double new_pos)
{
...
double SimuMotorController::ReadOne(long idx)
{
...
void SimuMotorController::StartOne(long idx,double new_pos)
{
	//cout << "[SimuMotorController] Starting one motor with index " << idx << " on controller SimuMotorController/" << inst_name << " (" << DevName << ")" << endl;
...
void SimuMotorController::StartAll()
{
	...
	if (simu_ctrl != NULL)
	{
		long nb_mot = wanted_mot.size();
		
		for (long loop = 0;loop < nb_mot;loop++)
		{
			Tango::DeviceData d_in;
...
void SimuMotorController::StateOne(long idx,Controller::CtrlState *info_ptr)
{
...
Controller::CtrlData SimuMotorController::GetPar(long idx,string &par_name)
{
...
void SimuMotorController::SetPar(long idx,string &par_name,Controller::CtrlData &new_value)
{
	...
	if (simu_ctrl != NULL)
	{
		...
		{
			if (new_value.data_type == Controller::LONG)	
				cout << "[SimuMotorController] New value for backlash is " << new_value.lo_data << endl;
			else
				bad_data_type(par_name);
		}
		else
...
Controller::CtrlData SimuMotorController::GetExtraAttributePar(long idx,string &par_name)
{
	Controller::CtrlData par_value;	

	if (par_name == "Aaa")
	{
		par_value.lo_data = 1234;
...
void SimuMotorController::SetExtraAttributePar(long idx,string &par_name,Controller::CtrlData &new_value)
{
	if (par_name == "Aaa")
	{
		if (new_value.data_type == LONG)
			cout << "[SimuMotorController] New value for Aaa extra attribute is " << new_value.lo_data << endl;
		else
			bad_data_type(par_name);		
	}
	else if (par_name == "Bbb")
...
long SimuMotorController_MaxDevice = 1024;
