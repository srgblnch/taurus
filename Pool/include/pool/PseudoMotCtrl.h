...
public:
	...
	virtual double CalcPhysical(long,std::vector<double> &) = 0;
	virtual double CalcPseudo(long,std::vector<double> &) = 0;
	...
	virtual void CalcAllPhysical(std::vector<double> &pseudo_pos, std::vector<double> &physical_pos)
	{
		...
		unsigned long m_nb = physical_pos.size();
		assert(m_nb > 0);
		
		for(unsigned long axis = 0; axis < m_nb ; axis++)
	...
	virtual void CalcAllPseudo(std::vector<double> &physical_pos, std::vector<double> &pseudo_pos)
	{
		// The physical_pos vector passed as argument should already
		// be initialized with the correct size.
		unsigned long pm_nb = pseudo_pos.size();
		assert(pm_nb > 0);
		
		for(unsigned long axis = 0; axis < pm_nb ; axis++)
