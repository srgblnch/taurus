...
PoolClass::~PoolClass()
{
	...
	if (python_initialised  == true)
	{	
		int th_id = omni_thread::self()->id();
		this->clear_locks(th_id);

//
// If a admin "Init" command is received as the first command for a thread,
// there is no lock for it
//

		{
			omni_mutex_lock lo(py_locks_mutex);		
			map<int,PythonLock *>::iterator map_ite = py_locks.find(th_id);
			if (map_ite == py_locks.end())
			{
				pair<map<int,PythonLock *>::iterator,bool> status;
				status = py_locks.insert(make_pair(th_id,new PythonLock(init_python_th_id,py_inter)));
				if (status.second == false)
				{
					...
					Tango::Except::throw_exception((const char *)"Pool_CantCreatePythonLock",o.str(),
        						       				(const char *)"PoolClass::~PoolClass()");	
				}
		...
		// Uncomment the following line if a trace of import and reload operations is required
		// Don't forget also to uncomment in PyExternalFile.cpp init_py_env() the corresponding code 
		/*
		PyObject *mod_dict = PyImport_GetModuleDict();

		PyObject *module_names = PyDict_Keys(mod_dict);
		
		int mod_count = PyList_Size(module_names);
		cout << "Python module report (" << mod_count << "):" << endl;
		
		for(int i = 0; i < mod_count; i++)
		{
			PyObject *module_name = PyList_GetItem(module_names,i);
			PyObject *module = PyDict_GetItem(mod_dict,module_name);
			cout << "Module " << PyString_AsString(module_name) << " has ref count=" << module->ob_refcnt << endl;
		}
		cout << "[DONE]" << endl;
		Py_DECREF(module_names);
		
		PyRun_SimpleString("mt.uninstall()");
		*/
				
		Py_Finalize();

		cout4 << "Python is finalized!!!!!" << endl;
		
		finalyse_python_th_id = th_id;
		{
			omni_mutex_lock lo(py_locks_mutex);		
			map<int,PythonLock *>::iterator map_ite = py_locks.find(th_id);
...
//+----------------------------------------------------------------------------
//
// method : 		PoolClass::CtrlType_2_str()
// 
// description : 	Return a string corresponding to the controller object
//					type given in parameter
//
// arg(s) : - type : The controller device type
//
//-----------------------------------------------------------------------------

string PoolClass::CtrlType_2_str(CtrlType type)
{
	string str;
	...
	default:
		str = "Unknown controller device type";
		break;
	}
...
vector<CtrlFiCa *>::iterator PoolClass::get_ctrl_fica_by_name(string &name,string &ob_type)
{
	...
	for (ite = ctrl_ficas.begin();ite != ctrl_ficas.end();++ite)
	{
		if ((*ite) != NULL)
		{
			...
			{
				CtrlType o_type = str_2_CtrlType(ob_type);
				...
				else
				{
					...
					o << CtrlType_2_str((*ite)->get_obj_type()) << ends;
...
//
// arg(s) : - name : The controller type (f_name/ctrl_class_name)
													  ...
													  Pool *dev)
{
	...
	else
	{
		...
		CtrlType ctrl_type = str_2_CtrlType(obj_type);
		CtrlFiCa *ptr_fica = NULL;
		switch (ctrl_type)
		{
			case MOTOR_CTRL:
			{
				ptr_fica = new MotCtrlFiCa(name,f_name,ctrl_class_name,
										   obj_type,this,dev);
				break;
			}
			...
			case PSEUDO_MOTOR_CTRL:
			{
				ptr_fica = new PseudoMotCtrlFiCa(name,f_name,ctrl_class_name,
												 obj_type,this,dev);
				break;
			}
			...
			case COTI_CTRL:
			{
				ptr_fica = new CoTiCtrlFiCa(name,f_name,ctrl_class_name,
											obj_type,this,dev);
				break;
			}
			...
			case ZEROD_CTRL:
			{
				ptr_fica = new ZeroDCtrlFiCa(name,f_name,ctrl_class_name,
											 obj_type,this,dev);
				break;
			}
			...
			case ONED_CTRL:
			{
				Tango::Except::throw_exception(
						...
						(const char *)"1D controllers not supported yet !!!",
						(const char *)"PoolClass::add_ctrl_fica");
			}
			...
			case TWOD_CTRL:
			{
				Tango::Except::throw_exception(
						(const char *)"Pool_BadCtrlType",
						(const char *)"2D controllers not supported yet !!!",
						(const char *)"PoolClass::add_ctrl_fica");
			}
			
			case PSEUDO_COUNTER_CTRL:
			{
				ptr_fica = new PseudoCoCtrlFiCa(name,f_name,ctrl_class_name,
												obj_type,this,dev);
				break;
			}
			...
			case COM_CTRL:
			{
				ptr_fica = new ComCtrlFiCa(name,f_name,ctrl_class_name,
										   obj_type,this,dev);
				break;
			}
			...
			{
				ptr_fica = new ConstraintFiCa(name,f_name,ctrl_class_name,
											  obj_type,this,dev);
				break;
			}
...
long PoolClass::get_fica_nb_by_f_name(string &wanted_f_name)
{
	vector<CtrlFiCa *>::iterator ite;
	long ctr = 0;
	for (ite = ctrl_ficas.begin();ite < ctrl_ficas.end();++ite)
...
long PoolClass::get_fica_nb_by_name(string &wanted_fica_name)
{
	vector<CtrlFiCa *>::iterator ite;
	long ctr = 0;
	for (ite = ctrl_ficas.begin();ite < ctrl_ficas.end();++ite)
...
//+----------------------------------------------------------------------------
//
// method : 		PoolClass::clear_locks()
// 
// description : 	Delete the object used to manage the pytho lock
//
// args : th : The thread actually calling this method and which will do a 
//		Py_Finalyse(). Do not delete the Python lock for this specific
//		one. This is done afterwards
//
//-----------------------------------------------------------------------------

void PoolClass::clear_locks(int th)
{
	if (py_locks.empty() == false)
	{
		map<int,PythonLock *>::iterator ite;
		for (ite = py_locks.begin();ite != py_locks.end();++ite)
		{
			if (ite->first != th)
			{
				delete ite->second;
...
vector<CtrlFile *>::iterator PoolClass::get_ctrl_file_by_name(string &f_name)
{
...
vector<CtrlFile *>::iterator PoolClass::add_ctrl_file(string &f_name,CtrlType ctrl_dev_type,Pool *dev)
{
	...
	else
	{
		...
		if (lang == CPP)
		{
			...
			{
				case MOTOR_CTRL:
					ptr_ext_file = new CppMotCtrlFile(f_name,this,dev);
					break;

				case PSEUDO_MOTOR_CTRL:
					ptr_ext_file = new CppPseudoMotCtrlFile(f_name,this,dev);
					break;
				
				case COTI_CTRL:
					ptr_ext_file = new CppCoTiCtrlFile(f_name,this,dev);
					break;
				
				case ZEROD_CTRL:
					ptr_ext_file = new CppZeroDCtrlFile(f_name,this,dev);
					break;
				...
				case PSEUDO_COUNTER_CTRL:
					ptr_ext_file = new CppPseudoCoCtrlFile(f_name,this,dev);
					break;
					
				case COM_CTRL:
					ptr_ext_file = new CppComCtrlFile(f_name,this,dev);
					break;
		...
		else
		{
			...
			{
				case MOTOR_CTRL:
					ptr_ext_file = new PyMotCtrlFile(f_name,this,dev);
					break;
	
				case PSEUDO_MOTOR_CTRL:
					ptr_ext_file = new PyPseudoMotCtrlFile(f_name,this,dev);
					break;
				
				case COTI_CTRL:
					ptr_ext_file = new PyCoTiCtrlFile(f_name,this,dev);
					break;
				
				case ZEROD_CTRL:
					ptr_ext_file = new PyZeroDCtrlFile(f_name,this,dev);
					break;
				...
				case PSEUDO_COUNTER_CTRL:
					ptr_ext_file = new PyPseudoCoCtrlFile(f_name,this,dev);
					break;
					
				case COM_CTRL:
					ptr_ext_file = new PyComCtrlFile(f_name,this,dev);
					break;
...
vector<string> &PoolClass::get_pool_path()
{
	...
	Pool *pool = static_cast<Pool *>(dev_list[0]);
	
	return pool->get_pool_path();
}

//+----------------------------------------------------------------------------
//
// method : 		PoolClass::init_py_env()
// 
// description : 	init Python environment if not already done.
//
//-----------------------------------------------------------------------------
void PoolClass::init_py_env()
{
	if (python_initialised == false)
	{
		Py_Initialize();
		...
		{
			cerr << "Python threads already improperly initialised by someone else !!!" << endl;
			init_python_th_id = omni_thread::self()->id();
			PyEval_AcquireLock();
			tstate = PyEval_SaveThread();
			py_inter = tstate->interp;
		...
		// Uncomment the following lines if a trace of import and reload operations is required
		// Don't forget also to uncomment in PoolClass.cpp desctructor the corresponding code 

		//PyRun_SimpleString("import ModuleTracer");
		//PyRun_SimpleString("mt = ModuleTracer.ModuleTracer('pool_py_module_trace.log')");
