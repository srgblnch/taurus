...
found_in_db(false),save_default(NULL)
{
	...
	{ /* nothing to do on string. value_str member already contains the value.*/ }
		...
		long_arr_v = new vector<long>(*cpy.long_arr_v);
...
void PropertyData::save_default_value()
{
	...
	{ /* nothing to do on string. value_str member already contains the value.*/ }
		...
		(*save_default->long_arr_v) = *long_arr_v;
	else if(type == Tango::DEVVAR_DOUBLEARRAY)
...
void PropertyData::restore_default_value()
{
	...
	{
		delete long_arr_v;
		long_arr_v = new vector<long>(*(save_default->long_arr_v));
	}
	else if(type == Tango::DEVVAR_DOUBLEARRAY)
	{
		delete double_arr_v;
...
PyObject *PyPropertyData::to_py()
{
	...
	{
		int len = long_arr_v->size();
		value = PyTuple_New(len);
		
		for(int i = 0; i < len; i++)
			PyTuple_SetItem(value, i, PyLong_FromLong((*long_arr_v)[i]));
	}
	else if(type == Tango::DEVVAR_DOUBLEARRAY)
	{ 
		int len = double_arr_v->size();
		value = PyTuple_New(len);
...
void PyPropertyData::from_py(PyObject *value)
{
	...
	else if(type == Tango::DEV_LONG)
	{
		...
		{
			long_v = PyLong_AsLong(value);
			toString(long_v, value_str);
		}
		else if(PyInt_Check(value))
		{
			long_v = PyInt_AsLong(value);
			toString(long_v, value_str);
		}
		else 
		{
	...
	else if(type == Tango::DEVVAR_LONGARRAY)
	{
		...
		else
		{
			...
			for(int i = 0; i < len; i++)
			{
				...
				{
					long_arr_v->push_back(PyLong_AsLong(valuel));	
				}
				else if(PyInt_Check(valuel))
				{
					long_arr_v->push_back(PyInt_AsLong(valuel));
				}
				else
				{
					type_mismatch = true;
...
//
	...
	case Tango::DEVVAR_BOOLEANARRAY:
		...
		if ((tmp_datum >> tmp_vect) == false)
		{
			type_mismatch = true;
			expected = "Boolean Array";
		}
		for (unsigned long loop = 0;loop < tmp_vect.size();loop++)
			(*bool_arr_v)[loop] = (bool)tmp_vect[loop];
		}
		break;
...
Controller::Properties CppPropertyData::to_cpp()
{
	...
	{
	case Tango::DEV_LONG:
		cp.value.long_prop.push_back(long_v);
		break;
