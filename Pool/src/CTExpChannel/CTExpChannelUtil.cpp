...
void CTExpChannelUtil::remove_object(Tango::Device_3Impl *dev)
{
	pool_dev->remove_ct(static_cast<CTExpChannel_ns::CTExpChannel *>(dev));
}

long CTExpChannelUtil::get_static_attr_nb(Tango::DeviceClass *cl_ptr)
{
	return (static_cast<CTExpChannel_ns::CTExpChannelClass *>(cl_ptr))->nb_static_attr;
}

void CTExpChannelUtil::add_2_full_name(Pool_ns::PoolElement *pe)
{
	pe->user_full_name = pe->user_full_name + " CounterTimer";
}
