...
PyCtrlFile::PyCtrlFile(string &f_name,const char *ctrl_dev_type,PoolClass *pool_class,Pool *dev)
:PyExternalFile(f_name,pool_class,dev),py_inter_lib_ptr(NULL),include_MaxDevice(true)
{
	string ctrl_type(ctrl_dev_type);
	ctrl_obj_type = pool_class->str_2_CtrlType(ctrl_type);
...
PyCtrlFile::PyCtrlFile(PyCtrlFile &undef_ctrl,const char *ctrl_dev_type,PoolClass *pool_class,Pool *dev)
:PyExternalFile(undef_ctrl,pool_class,dev),py_inter_lib_ptr(NULL),include_MaxDevice(true)
{
	string ctrl_type(ctrl_dev_type);
	ctrl_obj_type = pool_class->str_2_CtrlType(ctrl_type);
}
...
void PyCtrlFile::get_info(string &ctrl_class,vector<string> &info)
{
...
void PyCtrlFile::get_info(string &ctrl_class, string &ctrl_instance, 
						  vector<string> &info)
{
	AutoPythonLock apl = AutoPythonLock();
	...
	PyMotorController *my_ctrl = static_cast<PyMotorController *>(
					the_device->get_ctrl_from_inst_name(ctrl_instance).ctrl);
	if (my_ctrl != NULL)
	{
...
void PyCtrlFile::get_info_ex(string &ctrl_class, Tango::DevVarCharArray *info_ex)
{
	vector<string> info;
...
void PyCtrlFile::get_info_ex(string &ctrl_class, string &ctrl_instance, 
...
void PyCtrlFile::get_py_elem(string &class_name, const char *elem_name,
							 vector<string> &info, const char *dft)
{
	...
	PyObject *elem = PyObject_GetAttrString(class_obj,elem_name);
...
void PyCtrlFile::get_py_gender(string &class_name,vector<string> &info)
{
	get_py_elem(class_name,"gender",info,DEFAULT_GENDER);
}

void PyCtrlFile::get_py_model(string &class_name,vector<string> &info)
{
	get_py_elem(class_name,"model",info,DEFAULT_MODEL);
}

void PyCtrlFile::get_py_organization(string &class_name,vector<string> &info)
{
	get_py_elem(class_name,"organization",info,DEFAULT_ORGANIZATION);
}

void PyCtrlFile::get_py_image(string &class_name,vector<string> &info)
{
	get_py_elem(class_name,"image",info,"");
}

void PyCtrlFile::get_py_logo(string &class_name,vector<string> &info)
{
	get_py_elem(class_name,"logo",info,"");
}

void PyCtrlFile::get_py_icon(string &class_name,vector<string> &info)
{
...
//
//-----------------------------------------------------------------------------
void PyCtrlFile::load_py_inter_lib(string &dev_type)
{
    if (py_inter_lib_ptr == NULL)
	{
...
string PyCtrlFile::get_py_inter_lib_name(string &ctrl_dev_type)
{
...
long PyCtrlFile::get_MaxDevice(string &class_name)
{
	AutoPythonLock apl = AutoPythonLock();
...
// 
	...
	if (descr == NULL)
	{
		...
		descr = PyObject_CallMethod(pPyClass,"get_MaxDevice", NULL);
...
void PyCtrlFile::get_prop_info(string &class_name, vector<PropertyData*> &info)
{
...
//
	if(include_MaxDevice == true)
	{
		...
		long max_dev = get_MaxDevice(class_name);
		if (max_dev != -1)
		{
...
void PyCtrlFile::get_extra_attr_dec(string &class_name,const char *sym_name,vector<PoolExtraAttr> &sym_value)
{
	sym_value.clear();	
