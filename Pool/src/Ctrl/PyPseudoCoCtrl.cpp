...
PyPseudoCounterController::PyPseudoCounterController(const char *inst,
										   ...
										   PyObject *module,
										   PyObject *prop_dict):
PyController(),PseudoCounterController(inst),mod(module),py_class_name(cl_name)
{
	clear_method_flag();
...
//
//--------------------------------------------------------------------------
double PyPseudoCounterController::Calc(vector<double> &counter_values)
{
	PyObject *res;
	
	AutoPythonCtrlLock lo;
	
	long counter_nb = counter_values.size();
...
///
	for(int counter_index = 0 ; counter_index < counter_nb; counter_index++)
	{
...
///
	...
	if (!get_py_number(res,result))
	{
		Py_DECREF(res);
		Tango::Except::throw_exception(
			(const char *)"PyPseudoCoCtrl_BadReturnElementInTuple",
...
void PyPseudoCounterController::SetExtraAttributePar(long axis,string &par_name,
		...
		switch (par_value.data_type)
		{
			...
			case Controller::LONG:
   	 		res = PyObject_CallMethod(py_obj,
   	 				(char*)"SetExtraAttributePar", (char*)"lsl", axis,
   	 				par_name.c_str(), par_value.lo_data);
			break;
			
			case Controller::DOUBLE:
   	 		res = PyObject_CallMethod(py_obj,
   	 				(char*)"SetExtraAttributePar", (char*)"lsd", axis,
   	 				par_name.c_str(), par_value.db_data);
			break;
			
			default:
   	 		res = PyObject_CallMethod(py_obj,
   	 				(char*)"SetExtraAttributePar", (char*)"lss", axis,
...
Controller::CtrlData PyPseudoCounterController::GetExtraAttributePar(long axis,
...
long PyPseudoCounterController::get_counter_nb_from_py()
{
	long counter_nb;
